...
| **相对 R1 退出标准** | **进行中** | **~85–88%** | 内核+HTTP+集群数据面（多进程 raft M1–M3 + P4-01–P4-04）+ CI/烟雾合规 + W3C 子集 required-lite（30/30）+ 完整 W3C 套件 492/492 全绿 + 核心 SLO 实测达标（success 100%、p95=0ms）+ 恢复/灾备演练 DRILL PASS；剩余：正式验收包/OIDC 完整链路/实际回滚演练 |
...
| [~] 回滚演练通过 | 部分完成 | 发布/回滚手册已出（[L7-release-rollback.md](./L7-release-rollback.md)：代码级/数据级回滚流程与验证判据）；实际发布回滚演练待首次发布执行 |

**R1 判定：** 接近退出标准（约 **85–88%**；内核+HTTP+集群数据面+符合性+SLO+恢复演练齐备：W3C 492/492 全绿、多进程 raft 数据面落地、核心 SLO 实测达标、灾备演练 DRILL PASS；剩余缺口：正式验收包、OIDC 完整链路（R2+ 轨）、实际发布回滚演练待首次发布执行）。
...
- [x] **L7 平台工程**：P7-01/04 在线重平衡与灾备演练 **完成**（2026-08-08：`drill-rebalance-dr.sh` 真实 3 进程 raft 走完 7 步 DRILL PASS：选主→在线重平衡（slot bias 偏斜 + `shard_map_epoch` 前进证据）→复制收敛→杀 follower（多数派提交）→重启追赶→杀 leader（自动 failover）→重启追赶；cluster 30→31 测）✅；P7-02 阈值断言/趋势记录 **完成**（2026-08-08：`check-bench-thresholds.sh` 按 case ns/op 硬断言 + JSONL 趋势，CI `bench` 作业已切换）✅；P7-03 发布/回滚手册 **完成**（2026-08-08：[L7-release-rollback.md](./L7-release-rollback.md) 代码级/数据级回滚 + 验证判据）✅；P7-04 运维手册与证据包 **完成**（[L7-ops-rebalance-dr.md](./L7-ops-rebalance-dr.md)）✅ → **L7 全绿，光标移至 L8 AI-Native**
...
- [x] 发布流水线与回滚演练通过（P7-03）：[L7-release-rollback.md](./L7-release-rollback.md) 代码级/数据级回滚流程与验证判据（2026-08-08）
- [x] R1 退出标准全表勾选（2026-08-08）：SPARQL 查询基线 / 单区域集群核心 / 标准符合性门禁 / 核心 SLO 基线 / 恢复演练 已勾选完成；剩余 [~]：正式验收包（RDF 核心）、OIDC 完整链路（安全基线，R2+ 轨）、实际发布回滚演练（回滚基线，待首次发布）
...
7. [x] R1 退出标准全表勾选（2026-08-08；剩余 [~]：正式验收包 / OIDC 完整链路 R2+ / 实际回滚演练）
