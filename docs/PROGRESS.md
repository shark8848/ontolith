...
| P3-05 | MVP 标准符合性子集 | 部分完成 | 98% | 引擎单测 + [ontolith-compliance](../crates/ontolith-compliance) 17 烟雾 + W3C 子集运行器（must-pass 30/30，known-gap xfail=0，unsupported skip=0）+ **完整 W3C 套件 manifest 驱动 runner（`w3c11_suite`，492 条基线，265 PASS/227 FAIL 欠账 profile 化防回归）** + CI required-lite + strict observer + strict-promotion-readiness 自动信号 + `ci-local.sh` 全链路通过 | 观察主干连续 3 次 CI 全绿后评估 strict required；按 profile 欠账继续提 PASS（P3-05 已落地三阶段：函数/投影/算术/EXISTS/MINUS/聚合/CAST/构造语法 + 完整 datatype/lang 字面量模型，W3C 192→229→265 PASS；剩余 semantic 主导，需哈希函数/REPLACE/BNODE/图管理操作 ADD/COPY/MOVE 等） |
