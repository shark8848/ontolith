...
| ontolith-cluster | 选主、分区、复制、commit、rebalance、session sticky、数据面同步 + raft M1（内存存储/传输）+ raft M3（HTTP RPC + RocksDB raft CF + snapshot + 默认运行时切换 + 三进程 smoke）（27 测） | `crates/ontolith-cluster/src/infrastructure/{mod,raft}/**` |
...
- [~] **L4 集群**：P4-02 多进程 Raft **M1–M3 完成**（2026-08-08：M1 openraft 0.9.25 单节点引导 + trait 适配 + 内存传输，cluster 17→21 测；M2 多进程 HTTP RPC + RocksDB raft CF + snapshot install，cluster 21→26 测；M3 默认运行时切换 + CI 三进程 smoke + 真实复制语义，cluster 26→27 测）→ **P4-01 多进程 RPC**；P4-03 跨节点数据搬迁、P4-04 真实网络分区
...
5. [~] Phase 4 单区域集群最小闭环（**M1–M3 已落地**：单节点 raft 数据面 + 双/三节点 HTTP+RocksDB 复制 + 默认运行时切换 + CI 三进程 smoke；P4-01/P4-03/P4-04 后续）
