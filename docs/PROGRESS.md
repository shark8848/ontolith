...
对照代码快照: 2026-07-22（L0–L5 全量实现分批提交完成 + CI/合规烟雾 + W3C 子集门禁 required-lite + strict 观测轨 + 文件审计 + systemd 打包）；Git 当前头: `main` @ `84ea138`（COUNT 聚合基线已收敛，当前继续底层 L3 子查询能力收敛）
...
| Phase 3 查询引擎 | 部分完成 | ~86% | Turtle/TriG + SPARQL 核心代数/优化/绑定 + COUNT 聚合基线（无 GROUP BY）+ 子查询基线（嵌套 SELECT + LIMIT）+ W3C 子集门禁（required-lite，must-pass 12/12）；缺属性路径/完整聚合/Update |
...
| **分层内核 L0–L3** | **部分完成** | **~84–86%** | 语义+存储+查询主路径可用，COUNT 与子查询基线已纳入回归保护 |
| **相对 R1 退出标准** | **进行中** | **~69–72%** | 内核+HTTP+集群+CI/烟雾合规 + W3C 子集 required-lite；多节点数据面/W3C 全量/SLO 仍缺 |
...
| L3 parser/query | ~86% | 完整核心，非仅 MVP；COUNT 聚合+子查询基线已落地；W3C 子集 required-lite + strict 观测双轨 |
...
| ID | 交付物 | 状态 | 完成度 | 证据 | 下次动作 |
|----|--------|------|--------|------|----------|
| P3-01 | SPARQL 解析到执行主链路 | 部分完成 | 87% | SELECT/ASK/CONSTRUCT + JOIN/OPT/UNION/FILTER/BIND/VALUES + COUNT 聚合（无 GROUP BY）+ 子查询基线（嵌套 SELECT + LIMIT） | 属性路径/完整聚合 |
...
| P3-05 | MVP 标准符合性子集 | 部分完成 | 75% | 引擎单测 + [ontolith-compliance](../crates/ontolith-compliance) 15 烟雾 + W3C 子集运行器（must-pass 12/12，known-gap xfail=0，unsupported skip=2）+ CI required-lite + strict observer + `ci-local.sh` 全链路通过 | 扩展到 20–40 case；稳定后评估 strict required |
...
| [~] 标准符合性门禁通过 | 部分完成 | CI + R1 烟雾 15 测 + W3C 子集（required-lite，must-pass 12/12，xfail=0，xpass=0）+ strict observer（non-blocking）；无完整 W3C 套件 |
| [ ] 核心 SLO 基线达标 | 未完成 | 无基准 |
| [~] 恢复演练通过 | 部分完成 | RocksDB reopen 单测；无演练手册 |
| [ ] 回滚演练通过 | 未完成 | 无发布链路 |

**R1 判定：** 未达退出标准（约 **66–69%**；内核+HTTP+集群控制面可演示，`ci-local.sh` 全链路通过并纳入 W3C 子集 required-lite + strict observer 双轨；多节点数据面/符合性全量/SLO 仍缺）。
...
| WBS-04 | 查询与优化 | 部分完成 | ~81% | 完整核心代数+优化+绑定 + COUNT 聚合基线 + 子查询基线 + W3C 子集门禁；缺属性路径/完整聚合 |
...
| 门禁/治理项 | 状态 | 证据 / 缺口 |
|-------------|------|-------------|
| [~] RDF/SPARQL 标准测试 | 部分完成 | `ontolith-compliance` R1 烟雾 15 + W3C 子集运行器（must-pass 12/12，known-gap: xfail 0 / xpass 0，unsupported skip 2）+ CI required-lite / strict observer；非完整 W3C 官方 |
...
| ontolith-query | SELECT/ASK/CONSTRUCT、JOIN/OPTIONAL/UNION/FILTER/BIND/VALUES、COUNT 聚合（无 GROUP BY）、子查询基线（嵌套 SELECT + LIMIT）、Explain/timeout（25 测） | `crates/ontolith-query/src/infrastructure/**` |
