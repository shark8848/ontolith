...
| Phase 6 推理与验证 | 部分完成 | ~45% | 前向链推理引擎（rdfs5/6/7/8/9 + prp-inv1/2、prp-symp/trp、cax-sco，迭代上限 + 墙钟超时护栏）可用；**SHACL 基线校验引擎落地（目标/约束子集/逻辑形状/数值范围/属性对 + 验证报告，reasoner 4→31 测）** |
...
| ontolith-reasoner | 前向链推理（rdfs5/6/7/8/9 + prp-inv1/2、prp-symp/trp、cax-sco、`InferenceMode` 开关 + 迭代/超时护栏）+ SHACL 基线校验（目标选择/约束子集/逻辑形状/数值范围/属性对/qualified 计数/报告，31 测） | `crates/ontolith-reasoner/src/infrastructure/{mod,shacl}.rs` |
