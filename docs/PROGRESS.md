...
| **相对 R1 退出标准** | **进行中** | **~90%** | 内核+HTTP+集群数据面（多进程 raft M1–M3 + P4-01–P4-04）+ CI/烟雾合规 + W3C 子集 required-lite（30/30）+ 完整 W3C 套件 492/492 全绿 + 核心 SLO 实测达标（success 100%、p95=0ms）+ 恢复/灾备演练 DRILL PASS + **实际发布回滚演练 DRILL PASS**；剩余：正式验收包/OIDC 完整链路（R2+） |
...
**R1 判定：** 接近退出标准（约 **90%**；内核+HTTP+集群数据面+符合性+SLO+恢复+发布回滚演练齐备：W3C 492/492 全绿、多进程 raft 数据面落地、核心 SLO 实测达标、灾备演练 DRILL PASS、实际发布回滚演练 DRILL PASS；剩余缺口：正式验收包、OIDC 完整链路（R2+ 轨））。
...
- [x] R1 退出标准全表勾选（2026-08-08）：SPARQL 查询基线 / 单区域集群核心 / 标准符合性门禁 / 核心 SLO 基线 / 恢复演练 / **实际发布回滚演练**（2026-08-08：`release-rollback-drill.sh` staging DRILL PASS，回滚基线完成）已勾选完成；剩余 [~]：正式验收包（RDF 核心）、OIDC 完整链路（安全基线，R2+ 轨）
...
7. [x] R1 退出标准全表勾选（2026-08-08；实际发布回滚演练已完成，剩余 [~]：正式验收包 / OIDC 完整链路 R2+）
