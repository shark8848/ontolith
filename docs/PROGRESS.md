...
| Phase 6 推理与验证 | 部分完成 | ~48% | 前向链推理引擎（rdfs5/6/7/8/9 + prp-inv1/2、prp-symp/trp、cax-sco、cls-svf1/2、cls-avf，迭代上限 + 墙钟超时护栏）可用；**SHACL 基线校验引擎落地（目标/约束子集/逻辑形状/数值范围/属性对 + 验证报告，reasoner 4→34 测）** |
...
| ID | 交付物 | 状态 | 完成度 | 证据 | 下次动作 |
|----|--------|------|--------|------|----------|
| P6-01 | OWL 2 RL 核心规则 | 部分完成 | 60% | `ForwardChainReasoner`：rdfs5/6/7/8/9 + prp-inv1/2 + prp-symp + prp-trp + cax-sco + cls-svf1/2 + cls-avf（含 bnode 限定词）、迭代闭包、`InferenceMode` 开关、12 测 | 扩展规则集（cls-avl 列表限定词、prp-key、一致性 ⊥ 规则） |
...
| ontolith-reasoner | 前向链推理（rdfs5/6/7/8/9 + prp-inv1/2、prp-symp/trp、cax-sco、cls-svf1/2、cls-avf、`InferenceMode` 开关 + 迭代/超时护栏）+ SHACL 基线校验（目标选择/约束子集/逻辑形状/数值范围/属性对/qualified 计数/报告，34 测） | `crates/ontolith-reasoner/src/infrastructure/{mod,shacl}.rs` |
