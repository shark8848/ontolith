...
- [~] **L4 集群**：P4-02 多进程 Raft **M1 完成**（2026-08-08：openraft 0.9.25 单节点引导 + `RaftClusterRuntime` trait 适配 + 内存传输，cluster 17→21 测）→ **M2**（多进程 HTTP RPC + RocksDB raft CF + snapshot install）→ M3（默认运行时切换 + CI 三进程 smoke）；P4-01 多进程 RPC、P4-03 跨节点数据搬迁、P4-04 真实网络分区
...
- [~] 多节点数据面实施（**M1 单节点 openraft 适配完成**（2026-08-08，cluster 17→21 测）→ M2 多进程 HTTP RPC + RocksDB raft CF → M3 默认运行时切换 + CI 三进程 smoke）
...
5. [~] Phase 4 单区域集群最小闭环（M1 单节点 raft 数据面已落地；M2/M3 进行中，可与 3 并行）
