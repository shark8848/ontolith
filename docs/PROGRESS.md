...
| P1 | L4 集群多进程 Raft 实施（P4-02 **M1–M2 完成**：openraft 适配 → 多进程 HTTP RPC + RocksDB raft CF + snapshot install → M3 默认运行时切换 + CI 三进程 smoke） | TBD | TBD |
...
| P4-02 | Raft 控制基线 | 部分完成 | 75% | 任期/日志 + **commit_index 多数派**；[ADR-0004](../adr/0004-multi-process-raft-data-plane.md) **M1 openraft 适配 + M2 多进程 HTTP RPC + RocksDB raft CF + snapshot install**（cluster 21→26 测，双节点 HTTP+RocksDB 选举/复制/落盘） | M3 默认运行时切换 + CI 三进程 smoke |
...
| ontolith-cluster | 选主、分区、复制、commit、rebalance、session sticky、数据面同步 + raft M1（内存存储/传输）+ raft M2（HTTP RPC + RocksDB raft CF + snapshot）（26 测） | `crates/ontolith-cluster/src/infrastructure/{mod,raft}/**` |
...
- [~] **L4 集群**：P4-02 多进程 Raft **M1–M2 完成**（2026-08-08：M1 openraft 0.9.25 单节点引导 + trait 适配 + 内存传输，cluster 17→21 测；M2 多进程 HTTP RPC + RocksDB raft CF + snapshot install，cluster 21→26 测，双节点 HTTP+RocksDB 选举/复制/落盘）→ **M3**（默认运行时切换 + CI 三进程 smoke）；P4-01 多进程 RPC、P4-03 跨节点数据搬迁、P4-04 真实网络分区
...
- [~] 多节点数据面实施（**M1–M2 完成**（2026-08-08：M1 单节点 openraft 适配，cluster 17→21 测；M2 多进程 HTTP RPC + RocksDB raft CF + snapshot install，cluster 21→26 测，双节点 HTTP+RocksDB 选举/复制/落盘）→ M3 默认运行时切换 + CI 三进程 smoke）
...
5. [~] Phase 4 单区域集群最小闭环（**M1–M2 已落地**：单节点 raft 数据面 + 双节点 HTTP+RocksDB 复制；M3 默认运行时切换进行中，可与 3 并行）
