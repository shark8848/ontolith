...
| Phase 8 AI-Native 扩展 | 进行中 | ~25% | 立项 + P8-01 M1（`ontolith-ai` crate，8 测）+ **M2 server 接线**（`/semantic/search`·`/semantic/index`，server 54 测）+ **M3 RocksDB 持久化与增量更新（删改回流）**（storage 53 测、ai 13 测、server 57 测）+ **P8-02 检索 KPI 门禁**（热路径优化实测 top-10 < 1ms、`ontolith-compliance` 门禁 + CI `retrieval-gates` 作业 + 语义 bench 阈值/趋势，仿 P7-02） |
...
| **相对 R1–R4 全计划** | **进行中** | **~23%** | R1/R2 完成、R3 要素多数落地（L5/L7）、L8 进行中（P8-01 M1+M2+M3 + P8-02 检索 KPI 门禁） |
...
| L8 AI-Native | ~25% | P8-01 M1 语义核心 + M2 server 接线 + M3 RocksDB 持久化与增量更新；P8-02 检索 KPI 门禁（确定性/相关命中/延迟 < 1ms + CI 门禁）已收尾；P8-03 代理扩展点未开始 |
...
| P8-03 | 代理集成扩展点 | 未开始 | 0% | — | `crates/ontolith-plugin-api` 增 `Retrieval` 能力 + `AgentTool` 抽象；依赖 P8-01/P8-02（已满足） |
...
> 当前光标：**L8 AI-Native（2026-08-09：立项 + P8-01 M1–M3 完成 + P8-02 检索 KPI 门禁完成；下一步 P8-03 代理集成扩展点）**
