...
| ontolith-storage | 内存六索引 + 命名图六置换 + RocksDB 耐久（reopen/abort/delete）+ codec + 命名图匹配 + **幂等写入验证**（Put 去重/Delete no-op/重复 commit 拒绝，内存 4 测 + RocksDB 1 测）+ **租户 CF**（`tenant_cf_*` 字节级原语：put/get/delete/scan_prefix/write_batch，roundtrip + 重开持久）（54 测） | `crates/ontolith-storage/src/infrastructure/**` |
...
| 2026-08-09 | Codex | **L6 推理端到端 drill DONE**：新增 `scripts/drill-reasoning-rules.sh`（独立实例、memory 存储、enforced 鉴权、`?inference=forward` 每请求覆盖）——断言 `/inference` 精确返回 87 条规则，并验证代表性新规则真实触发：prp-eqp1/2+prp-spo1（等价属性双向）、prp-dom+scm-dom1/prp-rng+scm-rng1（域/值域超类传播）、cls-maxqc3/4（限定基数 1 → sameAs）、prp-key（hasKey → sameAs）、eq-ref（sameAs 自反）、rdfs4a/rdfs1/rdfs13（Resource/Datatype/Literal 定型）、cls-maxc1+dt-not-type（不一致标记）；30/30 PASS，release 二进制实测全绿 |
...
| 2026-08-09 | Codex | **L8 P8-01 M2 server 接线 DONE**：`ontolith-server` 接入 `ontolith-ai`——`SemanticConfig`（`ONTOLITH_SEMANTIC_ENABLED` 默认关 / `DIM` / `AUTO_INDEX_CAP`）+ `build_semantic_service`（启动自动索引：主语（bnode 跳过）/谓词/宾语，cap 100k）+ AppState.semantic（`Mutex<SemanticSearchService>`）；新增 `GET /semantic/search?q=&k=`（top-k 余弦命中，SPARQL 结果 JSON 同构 term 渲染 + 固定 6 位分数保证确定性）与 `POST /semantic/index?term=|<terms=>`（幂等去重）；鉴权/审计复用 L5 模式（`semantic:read/write`），未启用 501；`/health` 与 `/admin/config` 暴露 `semantic` 姿态；server 49→54 测（禁用 501/缺 q 400/缺 term 400/索引+检索往返+分数确定性/启动自动索引），workspace 全量 + clippy 零警告 + fmt 对齐；[L8-ai-native.md](./L8-ai-native.md) 0.1.0→0.1.1（env 契约 + API + 已知限制）；PROGRESS.md 0.1.38→0.1.39 |
| 2026-08-09 | Codex | **L8 看板增量同步 DONE（SYNC-PROJ-0001）**：GitHub Projects #2 新增 P8-01/02/03 三条（P8-01 进行中 / P8-02·P8-03 未开始，P2），L8 AI-Native 扩展转进行中；看板 total 46→49，回读验证通过 |
| 2026-08-09 | Codex | **看板同步 + L8 AI-Native 立项 DONE（SYNC-PROJ-0001 + P8-01 M1）**：① GitHub Projects #2 全量同步（46 条，44 更新 0 失败）：R2 四轨（代价模型/OWL 2 RL/Explain 门禁/推理护栏）+ R2 SHACL 缺口收尾 + P0-03 依赖登记 + P2-01 RocksDB 适配转 Done，新增「R2 退出标准全表勾选」「RDF 1.1 布尔项区分（uniqueLang-002 闭合）」两条，回读验证 total=46；② L8 AI-Native 立项：新文档 [L8-ai-native.md](./L8-ai-native.md)（范围/非目标、架构决策——可插拔 `EmbeddingProvider` + 树内确定性 fallback（零新增外部依赖）、语义索引与检索、R4 扩展安全与兼容门禁、M1–M4 里程碑、检索 KPI）；③ P8-01 语义-向量桥接 M1：新建 `crates/ontolith-ai`（`FeatureHashEmbedding`——FNV-1a 64 特征哈希 + token/字符三连 + L2 归一、`InMemorySemanticIndex`——幂等 upsert + 余弦 top-k + k∈[1,100] 上限、`SemanticSearchService`）；8 测全绿，workspace 全量 + clippy 零警告 + fmt 对齐；PROGRESS.md 0.1.37→0.1.38 |
...
| 2026-08-09 | Codex | **RDF 1.1 布尔项区分 + P2-01 索引 CF 调优 + P0-03 依赖登记审计 DONE**：① RDF 1.1 term distinctness——`"1"/"0"^^xsd:boolean` 不再词法折叠为 `Boolean(true/false)`（N-Triples/Turtle/SPARQL 三处 `coerce_typed_literal`），保留为 `Typed` 非规范词法项；SPARQL 值语义（RDFterm-equal）与 XSD cast（`xsd:boolean/integer/decimal/float/double/string` 六类，boolean→string 出规范形 `true/false`）按 XSD 值空间补齐；compliance harness 期望/实际两侧统一走 `norm_literal` 值归一；效果：**SHACL 唯一已知缺口 uniqueLang-002 闭合**（`sh:uniqueLang "1"^^xsd:boolean` 按规范不激活约束，97/98→**98/98 全绿**，profile 重新生成），W3C 492/492 零漂移；② P2-01 索引 CF 调优——`RocksDbOptions` 新增 `index_bloom_bits_per_key`（默认 10bits）/`index_block_cache_mb`（默认 64MB 共享 LRU）/`index_compression`（默认 LZ4，None/Snappy/Lz4/Zstd），六索引 CF 开库即应用，`tuning()` 姿态可观测，重开库持久 + 点查/前缀扫描正确性门禁（storage 51→52 测）；③ P0-03 依赖登记审计——`scripts/audit-dependency-register.sh` 校验工作区全部直接依赖已登记 `DEPENDENCY_REGISTER.md`（15 deps 全注册），CI 新增 `dependency-audit` 作业（审计硬门禁 + `cargo-audit` CVE 观测轨 non-blocking），登记表补 rustls/tonic-build/regex/rcgen/quick-xml/tempfile 行；全量 workspace 测全绿、clippy 零警告、fmt 对齐；PROGRESS.md 0.1.36→0.1.37 |
