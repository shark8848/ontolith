...
- 运行时架构实现与工程化流程。
- 安全、可观测性、发布流程与依赖治理。
- 管理平台（management plane）统一配置、监控、数据管理与运维闭环。
...
- 实现鉴权、租户隔离与审计日志。
- 实现可观测性基线（指标、追踪、日志）。
- 实现管理平台基线（独立管理服务、控制面 ACL、运行时连通性探测）。

交付物：
- 运行时安全基线。
- 租户隔离请求处理基线。
- 统一遥测基线。
- 管理平台 API 基线（配置/监控/数据管理）。
...
- SPARQL Update 基线（INSERT/DELETE DATA、DELETE·INSERT…WHERE、DELETE WHERE）。✅
- Explain、超时、取消。
...
- 网关边界。
- 鉴权、租户隔离、审计链路。
- 管理平台 API（配置、监控、数据管理）与 read/write ACL。
- 插件宿主与扩展生命周期。

### WBS-08 平台工程
- CI/CD、质量门禁、依赖审计、发布流程。
- 可观测性栈与运维手册。
- 管理平台 smoke 门禁与 probe/SLO 阈值治理。
...
- API 契约与错误语义可重复、可预测。
- 管理平台关键接口（`/admin/health`、`/admin/config`、`/admin/monitoring`）行为可重复且可验证。
...
- 性能回归超过阈值时阻断候选发布。
- 管理平台探测指标（probe 成功率/延迟）具备基线阈值并纳入回归跟踪。
...
- [x] 多节点数据面实施（**M1–M3 完成**，2026-08-08：M1 openraft 适配 + 单节点引导 + trait 适配 + 内存传输；M2 多进程 HTTP RPC（树内 HTTP/1.1 + 共享 secret）+ RocksDB raft CF + snapshot install，cluster 21→26 测，双节点 HTTP+RocksDB 选举/复制/落盘；M3 默认运行时切换 + 真实复制语义 + CI 三进程 smoke，cluster 26→27 测，三节点 HTTP+RocksDB 多数派提交/失一 follower 后仍可提交）
