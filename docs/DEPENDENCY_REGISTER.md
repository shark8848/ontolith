...
| `serde` / `serde_json` | B | Follow openraft's pin (serde 1.x in tree) | cluster; security; server | cluster: serialize raft RPC messages, log entries, hard state, and snapshots (M2); security: JWT claim payload encoding/parsing (P5-02); server: tenant registry JSON (tenant management) | Ecosystem-stable | Revert to openraft-only serde if a lighter codec is adopted |
| `rustls` / `rustls-pemfile` | A | Pin rustls 0.23 with `ring` provider (ADR-0003) | server | Management-plane TLS termination (rustls in-process) + R2 non-loopback TLS gate; PEM cert/key loading; **outbound `https://` client for remote `LOAD`/SPARQL `SERVICE`/JSON-LD remote `@context` (2026-09-09 Wave 4)** | Crypto/version churn; feature surface | Feature-off → plaintext loopback only (R2 gate rejects non-loopback without TLS); outbound falls back to `http://` only |
| `webpki-roots` | B | Pin exact in Cargo.lock (already pinned 0.26.11 via `ureq`) | server | Mozilla CA root set for the outbound `https://` fetcher (remote `LOAD`/`SERVICE`/JSON-LD remote `@context`); extra internal PKI via `ONTOLITH_REMOTE_FETCH_CA_BUNDLE` PEM | Root-set churn; trust-anchor policy | Env-off bundle + `http://`-only endpoints; swap to OS trust store later |
...
| `log-center-sdk` | B | Pin 0.1.x in Cargo.lock | server | Platform log delivery to ikc-log-center: `LogCenterClient` async batched `POST {endpoint}/ingest` (bounded queue + background sender, best-effort), env `LOG_CENTER_URL/TOKEN/TIMEOUT/QUEUE/BATCH` aligned with Python/Java SDKs; gateway access + startup logs structured with W3C trace_id/span_id | Outbound HTTP surface; best-effort silent drop semantics | Feature/env-off → stdout logging only (default when `LOG_CENTER_URL` unset) |
| `ureq` | B | Pin exact in Cargo.lock (already pinned 2.12.1 via `log-center-sdk`) | ai | Remote HTTP embedding provider (ADR-0006): `RemoteHttpEmbeddingProvider` POSTs OpenAI-compatible JSON to an external embedding endpoint with bearer API key + timeout; `json` feature enables `send_json`/`into_json` | Outbound HTTP surface; external service availability | Env-off → deterministic `FeatureHashEmbedding` (default when `ONTOLITH_SEMANTIC_EMBEDDING_URL` unset) |
