...
fn cases() -> Vec<W3cCase> {
    vec![
        ...
        W3cCase {
            id: "w3c-aggregate-group-by",
            source: "W3C SPARQL 1.1 Query tests (derived aggregate group by)",
            feature: "Aggregate GROUP BY",
            class: CaseClass::MustPass,
            reason: "full aggregation group-by count",
            strict_skip_exempt: false,
            format: DatasetFormat::NTriples,
            dataset: include_str!("w3c/data/basic.nt"),
            query: include_str!("w3c/queries/aggregate_group_by.rq"),
            expected: Some(ExpectedOutcome::SelectRows {
                rows: 3,
                vars: &["s", "c"],
            }),
        ...
        W3cCase {
            id: "w3c-aggregate-having",
            source: "W3C SPARQL 1.1 Query tests (derived aggregate having)",
            feature: "Aggregate HAVING",
            class: CaseClass::MustPass,
            reason: "full aggregation having filter over group aggregate",
            strict_skip_exempt: false,
            format: DatasetFormat::NTriples,
            dataset: include_str!("w3c/data/basic.nt"),
            query: include_str!("w3c/queries/aggregate_having.rq"),
            expected: Some(ExpectedOutcome::SelectRows {
                rows: 2,
                vars: &["s", "c"],
            }),
